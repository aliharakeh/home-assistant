...
@Composable
fun EditShareholderDialog(
    shareholder: Shareholder,
    onDismiss: () -> Unit,
    onSave: (Shareholder) -> Unit
) {
    var name by remember { mutableStateOf(shareholder.name) }

    // Initialize values based on existing shareholder data
    val initialShareType = when (shareholder.shareValue) {
        is ShareValue.Percentage -> ShareType.PERCENTAGE
        is ShareValue.CurrencyValue -> ShareType.CURRENCY
    }

    val initialPercentage = when (shareholder.shareValue) {
        is ShareValue.Percentage -> shareholder.shareValue.value.toString()
        else -> ""
    }

    val initialAmount = when (shareholder.shareValue) {
        is ShareValue.CurrencyValue -> shareholder.shareValue.amount.toString()
        else -> ""
    }

    val initialCurrency = when (shareholder.shareValue) {
        is ShareValue.CurrencyValue -> shareholder.shareValue.currency
        else -> Currency.USD
    }

    var shareType by remember { mutableStateOf(initialShareType) }
    var percentageValue by remember { mutableStateOf(initialPercentage) }
    var currencyAmount by remember { mutableStateOf(initialAmount) }
