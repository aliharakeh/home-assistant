...
@Composable
fun GlobalAddBillDialog(
    subscriptions: List<Subscription>,
    onDismiss: () -> Unit,
    onAdd: (subscriptionIndex: Int, bill: ElectricityBill) -> Unit
) {
    ...
    var selectedSubscriptionIndex by remember { mutableStateOf(0) }
    ...
    AlertDialog(
        ...
        title = { Text("Add Electricity Bill") },
        text = {
            Column(
                ...
                ExposedDropdownMenuBox(
                    ...
                    onExpandedChange = { subscriptionExpanded = !subscriptionExpanded }
                ) {
                                            OutlinedTextField(
                            value = if (subscriptions.isNotEmpty()) subscriptions[selectedSubscriptionIndex].name else "No subscriptions",
                            onValueChange = {},
                            readOnly = true,
                            label = { Text("Subscription") },
                            trailingIcon = { ExposedDropdownMenuDefaults.TrailingIcon(expanded = subscriptionExpanded) },
                            modifier = Modifier
                                .fillMaxWidth()
                                .menuAnchor()
                        )
        ...
        confirmButton = {
            TextButton(
                onClick = {
                    ...
                    if (amountValue != null && subscriptions.isNotEmpty()) {
                        onAdd(selectedSubscriptionIndex, ElectricityBill(amountValue, selectedCurrency, selectedDate))
                    }
                },
                enabled = subscriptions.isNotEmpty() && amount.toDoubleOrNull() != null
            ) {
                Text("Add")
            }
        },
